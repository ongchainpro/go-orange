...
// Verify that Client implements the orange interfaces.
var (
	_ = orange.ChainReader(&Client{})
	_ = orange.TransactionReader(&Client{})
	_ = orange.ChainStateReader(&Client{})
	_ = orange.ChainSyncReader(&Client{})
	_ = orange.ContractCaller(&Client{})
	_ = orange.GasEstimator(&Client{})
	_ = orange.GasPricer(&Client{})
	_ = orange.LogFilterer(&Client{})
	_ = orange.PendingStateReader(&Client{})
	// _ = orange.PendingStateEventer(&Client{})
	_ = orange.PendingContractCaller(&Client{})
)
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	// Create Orange Service
	config := &ongconfig.Config{Genesis: genesis}
	config.Ongash.PowMode = ongash.ModeFake
	ongservice, err := ong.New(n, config)
	if err != nil {
		t.Fatalf("can't create new orange service: %v", err)
	}
	// Import the test chain.
	if err := n.Start(); err != nil {
		t.Fatalf("can't start test node: %v", err)
	}
	if _, err := ongservice.BlockChain().InsertChain(blocks[1:]); err != nil {
...
func TestOngClient(t *testing.T) {
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == orange.NotFound {
		t.Fatal("error should not be nil/notfound")
	}
	// Test tx in block not found
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 1); err != orange.NotFound {
		t.Fatal("error should be orange.NotFound")
	}
}
...
func testChainID(t *testing.T, client *rpc.Client) {
	...
	if id == nil || id.Cmp(params.AllOngashProtocolChanges.ChainID) != 0 {
