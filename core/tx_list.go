...
// Remove deletes a transaction from the maintained map, returning whonger the
...
// prevent getting into and invalid state. This is not somonging that should ever
...
type txList struct {
	strict bool         // Whonger nonces are strictly continuous or not
...
// Overlaps returns whonger the transaction specified has the same nonce as one
// already contained within the list.
func (l *txList) Overlaps(tx *types.Transaction) bool {
	return l.txs.Get(tx.Nonce()) != nil
}

// Add tries to insert a new transaction into the list, returning whonger the
...
// Remove deletes a transaction from the maintained list, returning whonger the
...
// Empty returns whonger the list of transactions is empty or not.
...
// Underpriced checks whonger a transaction is cheaper than (or as cheap as) the
