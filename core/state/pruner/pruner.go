...
func NewPruner(db ongdb.Database, headHeader *types.Header, datadir, trieCachePath string, bloomSize uint64) (*Pruner, error) {
...
func prune(maindb ongdb.Database, stateBloom *stateBloom, middleStateRoots map[common.Hash]struct{}, start time.Time) error {
...
// in the disk.
func RecoverPruning(datadir string, db ongdb.Database, trieCachePath string) error {
...
func extractGenesis(db ongdb.Database, stateBloom *stateBloom) error {
...
func getHeadHeader(db ongdb.Database) (*types.Header, error) {
...
pruning. Remember don't start the Gong without deleting the clean trie cache
otherwise the entire database may be damaged!

Check the command description "gong snapshot prune-state --help" for more details.
`
