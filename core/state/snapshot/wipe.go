...
func wipeSnapshot(db ongdb.KeyValueStore, full bool) chan struct{} {
...
func wipeContent(db ongdb.KeyValueStore) error {
...
func wipeKeyRange(db ongdb.KeyValueStore, kind string, prefix []byte, keylen int) error {
	// Batch deletions togonger to avoid holding an iterator for too long
	var (
		batch = db.NewBatch()
		items int
	)
