...
type snapshot interface {
	...
	// Stale return whonger this layer has become stale (was flattened across) or
	// if it's still live.
	Stale() bool
...
// SnapshotTree is an Orange state snapshot tree. It consists of one persistent
...
func New(diskdb ongdb.KeyValueStore, triedb *trie.Database, cache int, root common.Hash, async bool, rebuild bool, recovery bool) (*Tree, error) {
...
func (t *Tree) Rebuild(root common.Hash) {
	...
	// Track whonger there's a wipe currently running and keep it alive if so
...
func (t *Tree) Verify(root common.Hash) error {
	...
	got, err := generateTrieRoot(nil, acctIt, common.Hash{}, stackTrieGenerate, func(db ongdb.KeyValueWriter, accountHash, codeHash common.Hash, stat *generateStats) (common.Hash, error) {
...
// generating is an internal helper function which reports whonger the snapshot
