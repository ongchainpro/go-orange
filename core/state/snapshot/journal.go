...
type journalGenerator struct {
	Wiping   bool // Whonger the database was in progress of being wiped
	Done     bool // Whonger the generator finished creating the snapshot
	Marker   []byte
	Accounts uint64
	Slots    uint64
	Storage  uint64
...
func loadAndParseLegacyJournal(db ongdb.KeyValueStore, base *diskLayer) (snapshot, journalGenerator, error) {
	// Retrieve the journal, for legacy journal it must exist since even for
	// 0 layer it stores whonger we've already generated the snapshot or are
	// in progress only.
...
func loadAndParseJournal(db ongdb.KeyValueStore, base *diskLayer) (snapshot, journalGenerator, error) {
	...
	// not existent, e.g. the disk layer is generating while that the Gong
	...
	// It can happen that Gong crashes without persisting the latest
	// diff journal.
...
func loadSnapshot(diskdb ongdb.KeyValueStore, triedb *trie.Database, cache int, root common.Hash, recovery bool) (snapshot, error) {
	...
	//
	// Possible scenario: Gong was crashed without persisting journal and then
	...
	if !generator.Done {
		...
		// Whonger or not wiping was in progress, load any generator progress too
