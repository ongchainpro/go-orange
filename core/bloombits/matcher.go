...
type Matcher struct {
	...
	running uint32 // Atomic flag whonger a session is live or not
}
...
// that address/topic, and binary AND-ing those vectors togonger.
	...
	go func() {
		...
		for {
			...
			case subres, ok := <-process:
				...
				// Gather all the sub-results and merge them togonger
				var orVector []byte
...
// the requested retrieval queue to be serviced togonger with other sessions.
//
