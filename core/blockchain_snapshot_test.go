...
func (basic *snapshotTestBasic) prepare(t *testing.T) (*BlockChain, []*types.Block) {
	...
	chain, err := NewBlockChain(db, cacheConfig, params.AllOngashProtocolChanges, engine, vm.Config{}, nil, nil)
	if err != nil {
...
// It can be used for testing that restart Gong normally.
...
func (snaptest *snapshotTest) test(t *testing.T) {
	...
	chain.Stop()
	newchain, err := NewBlockChain(snaptest.db, nil, params.AllOngashProtocolChanges, snaptest.engine, vm.Config{}, nil, nil)
	if err != nil {
...
// It can be used for testing that restart Gong after the crash.
...
func (snaptest *crashSnapshotTest) test(t *testing.T) {
	...
	newchain, err := NewBlockChain(newdb, nil, params.AllOngashProtocolChanges, snaptest.engine, vm.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
	newchain.Stop()
...
func (snaptest *gappedSnapshotTest) test(t *testing.T) {
	...
	newchain, err := NewBlockChain(snaptest.db, cacheConfig, params.AllOngashProtocolChanges, snaptest.engine, vm.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
...
func (snaptest *wipeCrashSnapshotTest) test(t *testing.T) {
	...
	newchain, err := NewBlockChain(snaptest.db, config, params.AllOngashProtocolChanges, snaptest.engine, vm.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
	newBlocks, _ := GenerateChain(params.TestChainConfig, blocks[len(blocks)-1], snaptest.engine, snaptest.gendb, snaptest.newBlocks, func(i int, b *BlockGen) {})
...
// Tests a Gong restart with valid snapshot. Before the shutdown, all snapshot
...
// Tests a Gong restart with valid but "legacy" snapshot. Before the shutdown,
...
// Tests a Gong was crashed and restarts with a broken snapshot. In this case the
...
// Tests a Gong was crashed and restarts with a broken snapshot. In this case
...
// Tests a Gong was crashed and restarts with a broken and "legacy format"
...
// Tests a Gong was running with snapshot enabled. Then restarts without
...
// Tests a Gong was running with leagcy snapshot enabled. Then restarts
...
// Tests the Gong was running with snapshot enabled and resetHead is applied.
...
// Tests the Gong was running with snapshot(legacy-format) enabled and resetHead
...
// Tests the Gong was running with snapshot(legacy-format) enabled and upgrades
// the disk layer journal(journal generator) to latest format. After that the Gong
// is restarted from a crash. In this case Gong will find the new-format disk layer
...
// Tests the Gong was running with a complete snapshot and then imports a few
