...
func NewDatabase(db ongdb.KeyValueStore) ongdb.Database {
	return &nofreezedb{
		KeyValueStore: db,
	}
}
...
// storage.
func NewDatabaseWithFreezer(db ongdb.KeyValueStore, freezer string, namespace string) (ongdb.Database, error) {
	...
	if kvgenesis, _ := db.Get(headerHashKey(0)); len(kvgenesis) > 0 {
		if frozen, _ := frdb.Ancients(); frozen > 0 {
			// If the freezer already contains somonging, ensure that the genesis blocks
...
func NewMemoryDatabase() ongdb.Database {
...
func NewMemoryDatabaseWithCap(size int) ongdb.Database {
	return NewDatabase(memorydb.NewWithCap(size))
}

// NewLevelDBDatabase creates a persistent key-value database without a freezer
// moving immutable chain segments into cold storage.
func NewLevelDBDatabase(file string, cache int, handles int, namespace string) (ongdb.Database, error) {
...
// freezer moving immutable chain segments into cold storage.
func NewLevelDBDatabaseWithFreezer(file string, cache int, handles int, freezer string, namespace string) (ongdb.Database, error) {
...
func InspectDatabase(db ongdb.Database, keyPrefix, keyStart []byte) error {
