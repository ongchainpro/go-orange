...
type rewindTest struct {
	...
	songeadBlock       uint64 // Block number to set head back to
...
// Tests a songead for a short canonical chain where a recent block was already
// committed to disk and then the songead called. In this case we expect the full
// chain to be rolled back to the committed block. Everything above the songead
...
// Tests a songead for a short canonical chain where the fast sync pivot point was
// already committed, after which songead was called. In this case we expect the
// chain to behave like in full sync mode, rolling back to the committed block
// Everything above the songead point should be deleted. In between the committed
...
// Tests a songead for a short canonical chain where the fast sync pivot point was
// not yet committed, but songead was called. In this case we expect the chain to
...
// Tests a songead for a short canonical chain and a shorter side chain, where a
...
// above the songead point should be deleted. In between the committed block and
...
// Tests a songead for a short canonical chain and a shorter side chain, where
...
// block. Everything above the songead point should be deleted. In between the
...
// Tests a songead for a short canonical chain and a shorter side chain, where
// the fast sync pivot point was not yet committed, but songead was called. In this
// test scenario the side chain is below the committed block. In this case we expect
...
// data above the songead point should be deleted. In between the committed block
...
// songead point should be deleted. In between the committed block and the
...
func testSetHead(t *testing.T, tt *rewindTest, snapshots bool) {
	...
	chain, err := NewBlockChain(db, config, params.AllOngashProtocolChanges, engine, vm.Config{}, nil, nil)
	if err != nil {
