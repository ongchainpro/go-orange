...
// Package core implements the Orange consensus protocol.
package core
...
type CacheConfig struct {
	...
	TrieCleanNoPrefetch bool          // Whonger to disable heuristic state prefetching for followup blocks
	TrieDirtyLimit      int           // Memory limit (MB) at which to start flushing dirty trie nodes to disk
	TrieDirtyDisabled   bool          // Whonger to disable trie write caching and GC altogonger (archive node)
...
type BlockChain struct {
	...
	db     ongdb.Database // Low level persistent database to store final content in
...
// available in the database. It initialises the default Orange Validator and
// Processor.
func NewBlockChain(db ongdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(block *types.Block) bool, txLookupLimit *uint64) (*BlockChain, error) {
	if cacheConfig == nil {
	...
	// the head block (ongash cache or clique voting snapshot). Might as well do
	// it in advance.
...
// empty returns an indicator whonger the blockchain is empty.
...
// persistent disk layer. Depending on whonger the node was fast synced or full, and
...
func (bc *BlockChain) SetHeadBeyondRoot(head uint64, root common.Hash) (uint64, error) {
	...
	updateFn := func(db ongdb.KeyValueWriter, header *types.Header) (uint64, bool) {
		...
		if currentBlock := bc.CurrentBlock(); currentBlock != nil && header.Number.Uint64() <= currentBlock.NumberU64() {
			...
			if newHeadBlock == nil {
				...
				// root hash
				beyondRoot := (root == common.Hash{}) // Flag whonger we're beyond the requested root (no root, always true)
		...
		// between the stateful-block and the songead target.
		var wipe bool
		if head+1 < frozen {
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts, ancientLimit uint64) (int, error) {
	...
	// and returns an indicator whonger the inserted blocks are canonical.
	...
	writeAncient := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		for i, block := range blockChain {
			...
			// Migrate all ancient blocks. This can happen if someone upgrades from Gong
			...
			//
			// But if the `TxIndexTail` is not nil, e.g. Gong is initialized with
	...
	if bc.cacheConfig.TrieDirtyDisabled {
		...
		if current := block.NumberU64(); current > TriesInMemory {
			...
			if nodes > limit || imgs > 4*1024*1024 {
				triedb.Cap(limit - ongdb.IdealBatchSize)
			}
...
// sync, Gong will automatically construct the missing indices and delete
// the extra indices.
func (bc *BlockChain) maintainTxIndex(ancients uint64) {
	defer bc.wg.Done()

	// Before starting the actual maintenance, we need to handle a special case,
	// where user might init Gong with an external ancient database. If so, we
	...
	indexBlocks := func(tail *uint64, head uint64, done chan struct{}) {
		...
		// If the user just upgraded Gong to a new version which supports transaction
...
//
// The verify parameter can be used to fine tune whonger nonce verification
