...
type Wallet struct {
	...
	deriveChain     orange.ChainStateReader   // Blockchain state reader to discover used account with
...
func (w *Wallet) Open(passphrase string) error {
	...
	if !w.session.paired() {
		...
		if pairing := w.Hub.pairing(w); pairing != nil {
			...
			// but we can't return ErrPINNeeded directly here because we don't know whonger
			// a PIN check or a PIN reset is needed.
			passphrase = ""
		}
	}
	// The smart card was successfully paired, retrieve its status to check whonger
	...
	for errc == nil && err == nil {
		...
		for i := 0; i < len(nextAddrs); i++ {
			for empty := false; !empty; {
				// Retrieve the next derived Orange account
...
// Contains returns whonger an account is part of this particular wallet or not.
...
// chain state reader.
func (w *Wallet) SelfDerive(bases []accounts.DerivationPath, chain orange.ChainStateReader) {
	w.lock.Lock()
	defer w.lock.Unlock()
...
type Session struct {
	...
	verified bool                  // Whonger the pin has been verified in this session.
}
...
type walletStatus struct {
	...
	Initialized   bool // Whonger the card has been initialized with a private key
}
