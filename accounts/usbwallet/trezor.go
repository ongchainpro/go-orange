...
import (
	...
	"github.com/ong2020/go-orange/accounts/usbwallet/trezor"
...
type trezorDriver struct {
	...
	pinwait        bool          // Flags whonger the device is waiting for PIN entry
	passphrasewait bool          // Flags whonger the device is waiting for passphrase entry
...
// Status implements accounts.Wallet, always whonger the Trezor is opened, closed
// or whonger the Orange app was not started on it.
...
// Orange address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.OrangeAddress)
	if _, err := w.trezorExchange(&trezor.OrangeGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.OrangeTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.OrangeTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
