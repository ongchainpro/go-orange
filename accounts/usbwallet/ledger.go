...
// https://raw.githubusercontent.com/LedgerHQ/blue-app-ong/master/doc/ongapp.asc
...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and Orange address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an Orange transaction after having the user validate the parameters
...
type ledgerDriver struct {
	...
	browser bool          // Flag whonger the Ledger is in browser mode (reply channel mismatch)
...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "Orange app in browser mode", w.failure
	}
	if w.offline() {
		return "Orange app offline", w.failure
	}
	return fmt.Sprintf("Orange app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}

// offline returns whonger the wallet and the Orange app is offline or not.
//
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// Orange app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the Orange app's version, will fail prior to v1.0.2
...
// and returning the Orange address located on that derivation path.
...
// ledgerVersion retrieves the current version of the Orange wallet app running
...
// ledgerDerive retrieves the currently active Orange address from a Ledger
...
//   Orange address length | 1 byte
//   Orange address        | 40 bytes hex ascii
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the Orange hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex sting into an Orange address and return
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// Create the transaction RLP based on whonger legacy or EIP155 signing was requested
	var (
		txrlp []byte
		err   error
	)
	...
	// Extract the Orange signature and do a sanity validation
