...
func TestSimulatedBackend(t *testing.T) {
	...
	if err != orange.NotFound {
		t.Fatalf("err should be `orange.NotFound` but received %v", err)
	}
...
func TestNewSimulatedBackend(t *testing.T) {
	...
	if sim.config != params.AllOngashProtocolChanges {
		t.Errorf("expected sim config to equal params.AllOngashProtocolChanges, got %v", sim.config)
	}

	if sim.blockchain.Config() != params.AllOngashProtocolChanges {
		t.Errorf("expected sim blockchain config to equal params.AllOngashProtocolChanges, got %v", sim.config)
	}
...
func TestSimulatedBackend_EstimateGas(t *testing.T) {
	...
	sim := NewSimulatedBackend(core.GenesisAlloc{addr: {Balance: big.NewInt(params.Oranger)}}, 10000000)
	defer sim.Close()
	...
	var cases = []struct {
		name        string
		message     orange.CallMsg
		expect      uint64
		expectError error
		expectData  interface{}
	}{
		{"plain transfer(valid)", orange.CallMsg{
			From:     addr,
			To:       &addr,
			Gas:      0,
		...
		{"plain transfer(invalid)", orange.CallMsg{
			From:     addr,
			To:       &contractAddr,
			Gas:      0,
			GasPrice: big.NewInt(0),
...
func TestSimulatedBackend_EstimateGasWithPrice(t *testing.T) {
	...
	sim := NewSimulatedBackend(core.GenesisAlloc{addr: {Balance: big.NewInt(params.Oranger*2 + 2e17)}}, 10000000)
	defer sim.Close()
	...
	var cases = []struct {
		...
		{"EstimateWithoutPrice", orange.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
			GasPrice: big.NewInt(0),
		...
		{"EstimateWithPrice", orange.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
		...
		{"EstimateWithVeryHighPrice", orange.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
		...
		{"EstimateWithSuperhighPrice", orange.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
...
func TestSimulatedBackend_PendingAndCallContract(t *testing.T) {
	...
	res, err := sim.PendingCallContract(bgCtx, orange.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	})
	...
	res, err = sim.CallContract(bgCtx, orange.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	}, nil)
...
func TestSimulatedBackend_CallContractRevert(t *testing.T) {
	...
	call[0] = func(input []byte) ([]byte, error) {
		return sim.PendingCallContract(bgCtx, orange.CallMsg{
			From: testAddr,
			To:   &addr,
			Data: input,
		})
	...
	call[1] = func(input []byte) ([]byte, error) {
		return sim.CallContract(bgCtx, orange.CallMsg{
			From: testAddr,
			To:   &addr,
			Data: input,
		}, nil)
