...
const schema string = `
    ...
    scalar Bytes32
    # Address is a 20 byte Orange address, represented as 0x-prefixed hexadecimal.
    scalar Address
    ...
    # Account is an Orange account at a particular block.
    type Account {
    ...
    # Transaction is an Orange transaction.
    type Transaction {
    ...
    type Block {
        ...
        # Account fetches an Orange account at the current block's state.
    ...
    type Pending {
      ...
      transactions: [Transaction!]
      # Account fetches an Orange account for the pending state.
      account(address: Address!): Account!
    ...
    type Query {
        # Block fetches an Orange block by number or by hash. If neither is
