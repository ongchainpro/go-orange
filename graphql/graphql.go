...
// Package graphql provides a GraphQL interface to Orange node data.
package graphql
...
// Account represents an Orange account at a particular block.
type Account struct {
	backend       ongapi.Backend
...
// Transaction represents an Orange transaction.
...
// Block represents an Orange block.
...
// `Log` objects.
func runFilter(ctx context.Context, be ongapi.Backend, filter *filters.Filter) ([]*Log, error) {
...
type CallData struct {
	From     *common.Address // The Orange address the call is from.
	To       *common.Address // The Orange address the call is to.
...
func (b *Block) Call(ctx context.Context, args struct {
	...
	result, err := ongapi.DoCall(ctx, b.backend, args.Data, *b.numberOrHash, nil, vm.Config{}, 5*time.Second, b.backend.RPCGasCap())
	if err != nil {
		return nil, err
	}
	status := Long(1)
...
func (b *Block) EstimateGas(ctx context.Context, args struct {
	...
	gas, err := ongapi.DoEstimateGas(ctx, b.backend, args.Data, *b.numberOrHash, b.backend.RPCGasCap())
	return Long(gas), err
}
...
func (p *Pending) Call(ctx context.Context, args struct {
	...
	pendingBlockNr := rpc.BlockNumberOrHashWithNumber(rpc.PendingBlockNumber)
	result, err := ongapi.DoCall(ctx, p.backend, args.Data, pendingBlockNr, nil, vm.Config{}, 5*time.Second, p.backend.RPCGasCap())
	if err != nil {
		return nil, err
	}
	status := Long(1)
...
func (p *Pending) EstimateGas(ctx context.Context, args struct {
	...
	pendingBlockNr := rpc.BlockNumberOrHashWithNumber(rpc.PendingBlockNumber)
	gas, err := ongapi.DoEstimateGas(ctx, p.backend, args.Data, pendingBlockNr, p.backend.RPCGasCap())
	return Long(gas), err
}
...
func (r *Resolver) SendRawTransaction(ctx context.Context, args struct{ Data hexutil.Bytes }) (common.Hash, error) {
	...
	hash, err := ongapi.SubmitTransaction(ctx, r.backend, tx)
	return hash, err
}
