...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for ongstats availability")
	if infos, err := checkOngstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["ongstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["ongstats"] = infos.Report()
		ongstats = infos.config
	}
