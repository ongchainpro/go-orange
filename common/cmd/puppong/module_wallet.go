...
FROM puppong/wallet:latest

ADD genesis.json /genesis.json

RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'gong --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'exec gong --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --ongstats \'{{.Ongstats}}\' --cache=512 --http --http.addr=0.0.0.0 --http.corsdomain "*" --http.vhosts "*"' >> wallet.sh

RUN \
	sed -i 's/PuppongNetworkID/{{.NetworkID}}/g' dist/js/ongerwallet-master.js && \
	sed -i 's/PuppongNetwork/{{.Network}}/g'     dist/js/ongerwallet-master.js && \
	sed -i 's/PuppongDenom/{{.Denom}}/g'         dist/js/ongerwallet-master.js && \
	sed -i 's/PuppongHost/{{.Host}}/g'           dist/js/ongerwallet-master.js && \
	sed -i 's/PuppongRPCPort/{{.RPCPort}}/g'     dist/js/ongerwallet-master.js
...
// checkWallet does a health-check against web wallet server to verify whonger
// it's running, and if yes, whonger it's responsive.
