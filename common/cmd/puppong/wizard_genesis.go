...
func (w *wizard) makeGenesis() {
	...
	fmt.Println(" 1. Ongash - proof-of-work")
	fmt.Println(" 2. Clique - proof-of-authority")

	choice := w.read()
	switch {
	case choice == "1":
		// In case of ongash, we're pretty much done
		genesis.Config.Ongash = new(params.OngashConfig)
...
// importGenesis imports a Gong genesis spec into puppong.
...
func (w *wizard) manageGenesis() {
	// Figure out whonger to modify or export the genesis
	fmt.Println()
	...
	case "2":
		...
		// Export the native genesis spec used by puppong and Gong
		gongJson := filepath.Join(folder, fmt.Sprintf("%s.json", w.network))
		if err := ioutil.WriteFile(gongJson, out, 0644); err != nil {
			log.Error("Failed to save genesis file", "err", err)
			return
		}
		log.Info("Saved native genesis chain spec", "path", gongJson)

		// Export the genesis spec used by Along (formerly C++ Orange)
		...
		// Export the genesis spec used by Harmony (formerly OrangeJ)
