...
// nodeDockerfile is the Dockerfile required to run an Orange node.
var nodeDockerfile = `
FROM orange2020/client-go:latest
...
{{end}}
RUN \
  echo 'gong --cache 512 init /genesis.json' > gong.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.orange2020/keystore/ && cp /signer.json /root/.orange2020/keystore/' >> gong.sh && \{{end}}
	echo $'exec gong --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --ongstats \'{{.Ongstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Orangerbase}}--miner.ongerbase {{.Orangerbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> gong.sh

ENTRYPOINT ["/bin/sh", "gong.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an Orange node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.orange{{if .Ongashdir}}
      - {{.Ongashdir}}:/root/.ongash{{end}}
    environment:
      - PORT={{.Port}}/tcp
      - TOTAL_PEERS={{.TotalPeers}}
      - LIGHT_PEERS={{.LightPeers}}
      - STATS_NAME={{.Ongstats}}
      - MINER_NAME={{.Orangerbase}}
...
// deployNode deploys a new Orange node container to a remote machine via SSH,
// docker and docker-compose. If an instance with the specified network name
// already exists there, it will be overwritten!
func deployNode(client *sshClient, network string, bootnodes []string, config *nodeInfos, nocache bool) ([]byte, error) {
	kind := "sealnode"
	if config.keyJSON == "" && config.ongerbase == "" {
		kind = "bootnode"
	...
	template.Must(template.New("").Parse(nodeDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"Ongstats":    config.ongstats,
		"Orangerbase": config.ongerbase,
	...
	template.Must(template.New("").Parse(nodeComposefile)).Execute(composefile, map[string]interface{}{
		...
		"Ongashdir":   config.ongashdir,
		"Network":     network,
		"Port":        config.port,
		"TotalPeers":  config.peersTotal,
		"Light":       config.peersLight > 0,
		"LightPeers":  config.peersLight,
		"Ongstats":    config.ongstats[:strings.Index(config.ongstats, ":")],
		"Orangerbase": config.ongerbase,
...
func (info *nodeInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Ongstats username":        info.ongstats,
	}
	if info.gasTarget > 0 {
		...
		if info.ongerbase != "" {
			// Ongash proof-of-work miner
			report["Ongash directory"] = info.ongashdir
			report["Miner account"] = info.ongerbase
		}
		if info.keyJSON != "" {
...
// whonger it's running, and if yes, whonger it's responsive.
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 gong --exec admin.nodeInfo.enode --cache=16 attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.orange"],
		ongashdir:  infos.volumes["/root/.ongash"],
		port:       port,
		peersTotal: totalPeers,
		peersLight: lightPeers,
		ongstats:   infos.envvars["STATS_NAME"],
