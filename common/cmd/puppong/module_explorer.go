...
FROM puppong/blockscout:latest

ADD genesis.json /genesis.json
RUN \
  echo 'gong --cache 512 init /genesis.json' > explorer.sh && \
  echo $'gong --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.OngPort}} --bootnodes {{.Bootnodes}} --ongstats \'{{.Ongstats}}\' --cache=512 --http --http.api "net,web3,ong,shh,debug" --http.corsdomain "*" --http.vhosts "*" --ws --ws.origins "*" --exitwhensynced' >> explorer.sh && \
  echo $'exec gong --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.OngPort}} --bootnodes {{.Bootnodes}} --ongstats \'{{.Ongstats}}\' --cache=512 --http --http.api "net,web3,ong,shh,debug" --http.corsdomain "*" --http.vhosts "*" --ws --ws.origins "*" &' >> explorer.sh && \
...
services:
    explorer:
        ...
        ports:
            - "{{.OngPort}}:{{.OngPort}}"
            - "{{.OngPort}}:{{.OngPort}}/udp"{{if not .VHost}}
            - "{{.WebPort}}:4000"{{end}}
        environment:
            - ONG_PORT={{.OngPort}}
            - ONG_NAME={{.OngName}}
...
func deployExplorer(client *sshClient, network string, bootnodes []string, config *explorerInfos, nocache bool, isClique bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(explorerDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"Ongstats":  config.node.ongstats,
		"OngPort":   config.node.port,
	})
	...
	template.Must(template.New("").Parse(explorerComposefile)).Execute(composefile, map[string]interface{}{
		...
		"DBDir":       config.dbdir,
		"OngPort":     config.node.port,
		"OngName":     config.node.ongstats[:strings.Index(config.node.ongstats, ":")],
		"WebPort":     config.port,
...
func (info *explorerInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Orange listener port ":  strconv.Itoa(info.node.port),
		"Ongstats username":      info.node.ongstats,
	}
	return report
}
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	p2pPort := infos.portmap[infos.envvars["ONG_PORT"]+"/tcp"]
	...
	stats := &explorerInfos{
		node: &nodeInfos{
			datadir:  infos.volumes["/opt/app/.orange"],
			port:     infos.portmap[infos.envvars["ONG_PORT"]+"/tcp"],
			ongstats: infos.envvars["ONG_NAME"],
		},
