...
FROM orange2020/client-go:alltools-latest
...
ENTRYPOINT [ \
	"faucet", "--genesis", "/genesis.json", "--network", "{{.NetworkID}}", "--bootnodes", "{{.Bootnodes}}", "--ongstats", "{{.Ongstats}}", "--ongport", "{{.OngPort}}",     \
...
func (info *faucetInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Funding amount (base tier)":   fmt.Sprintf("%d Orangers", info.amount),
...
// checkFaucet does a health-check against a faucet server to verify whonger
