...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"ongstats", "explorer", "wallet", "faucet"} {
		...
		for _, server := range available[service] {
			...
			switch service {
			case "ongstats":
				if infos, err := checkOngstats(client, w.network); err == nil {
					port = infos.port
				}
			case "explorer":
	...
	// If we have ongstats running, ask whonger to make the secret public or not
	if w.conf.ongstats != "" {
		fmt.Println()
		fmt.Println("Include ongstats secret on dashboard (y/n)? (default = yes)")
