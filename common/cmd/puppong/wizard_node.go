...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.ongstats == "" {
		log.Error("No ongstats server configured")
		return
	}
	...
	if w.conf.Genesis.Config.Ongash != nil && !boot {
		fmt.Println()
		if infos.ongashdir == "" {
			fmt.Printf("Where should the ongash mining DAGs be stored on the remote machine?\n")
			infos.ongashdir = w.readString()
		} else {
			fmt.Printf("Where should the ongash mining DAGs be stored on the remote machine? (default = %s)\n", infos.ongashdir)
			infos.ongashdir = w.readDefaultString(infos.ongashdir)
		}
	}
	...
	if infos.ongstats == "" {
		...
		infos.ongstats = w.readString() + ":" + w.conf.ongstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.ongstats)
		infos.ongstats = w.readDefaultString(infos.ongstats) + ":" + w.conf.ongstats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.Ongash != nil {
			// Ongash based miners only need an ongerbase to mine against
			fmt.Println()
			if infos.ongerbase == "" {
				fmt.Printf("What address should the miner use?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.ongerbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner use? (default = %s)\n", infos.ongerbase)
				infos.ongerbase = w.readDefaultAddress(common.HexToAddress(infos.ongerbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy Orange node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
