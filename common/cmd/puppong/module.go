...
// returned from docker inspect, parsed into a form easily usable by puppong.
type containerInfos struct {
	running bool              // Flag whonger the container is running currently
...
func inspectContainer(client *sshClient, container string) (*containerInfos, error) {
	// Check whonger there's a container running for the service
