...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many Orangers to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.ongstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.ongstats = w.readString() + ":" + w.conf.ongstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.ongstats)
		infos.node.ongstats = w.readDefaultString(infos.node.ongstats) + ":" + w.conf.ongstats
	}
