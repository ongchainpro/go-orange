...
* I want to auto-approve transaction to contract `OngAlarmClock` with `data`=`0xdeadbeef`, if `value=0`, `gas < 44k` and `gasPrice < 40Gwei`
...
implemented for `gong`. There are no known security vulnerabilities in, nor have we had any security-problems with it so far.
...
It's unclear whonger any other DSL could be more secure; since there's always the possibility of erroneously implementing a rule.
