...
import (
	...
	"github.com/ong2020/go-orange/contracts/checkpointoracle"
...
func sign(ctx *cli.Context) error {
	var (
		offline bool // The indicator whonger we sign checkpoint by offline.
		chash   common.Hash
		cindex  uint64
	...
	if !ctx.GlobalIsSet(nodeURLFlag.Name) {
		...
		head, err := ongclient.NewClient(node).HeaderByNumber(reqCtx, nil)
		if err != nil {
			return err
		}
	...
	// isAdmin checks whonger the specified signer is admin.
...
func publish(ctx *cli.Context) error {
	...
	head, err := ongclient.NewClient(client).HeaderByNumber(reqCtx, nil)
	if err != nil {
		return err
	}
	num := head.Number.Uint64()
	recent, err := ongclient.NewClient(client).HeaderByNumber(reqCtx, big.NewInt(int64(num-128)))
	if err != nil {
		return err
	}
