...
func TestMessageSignVerify(t *testing.T) {
	tmpdir, err := ioutil.TempDir("", "ongkey-test")
	if err != nil {
	...
	// Create the key.
	generate := runOngkey(t, "generate", "--lightkdf", keyfile)
	generate.Expect(`
...
`)
	...
	// Sign a message.
	sign := runOngkey(t, "signmessage", keyfile, message)
	sign.Expect(`
!! Unsupported terminal, password will be echoed.
Password: {{.InputLine "foobar"}}
`)
	...
	// Verify the message.
	verify := runOngkey(t, "verifymessage", address, signature, message)
