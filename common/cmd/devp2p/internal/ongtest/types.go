...
import (
	...
	"github.com/ong2020/go-orange/internal/utesting"
...
func (c *Conn) handshake(t *utesting.T) Message {
	...
	case *Hello:
		...
		c.negotiateOngProtocol(msg.Caps)
		if c.ongProtocolVersion == 0 {
...
// negotiateOngProtocol sets the Conn's ong protocol version
// to highest advertised capability from peer
func (c *Conn) negotiateOngProtocol(caps []p2p.Cap) {
	var highestOngVersion uint
	for _, capability := range caps {
		if capability.Name != "ong" {
			continue
		}
		if capability.Version > highestOngVersion && capability.Version <= 65 {
			highestOngVersion = capability.Version
		}
	}
	c.ongProtocolVersion = highestOngVersion
}
...
loop:
	...
	if status == nil {
		...
		status = &Status{
			ProtocolVersion: uint32(c.ongProtocolVersion),
...
func (c *Conn) waitForBlock(block *types.Block) error {
	...
	c.SetReadDeadline(timeout)
	for {
		req := &GetBlockHeaders{Origin: ong.HashOrNumber{Hash: block.Hash()}, Amount: 1}
