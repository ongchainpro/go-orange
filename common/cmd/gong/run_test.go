...
func tmpdir(t *testing.T) string {
	dir, err := ioutil.TempDir("", "gong-test")
	if err != nil {
		t.Fatal(err)
	}
	return dir
...
func init() {
	// Run the app if we've been exec'd as "gong-test" in runGong.
	reexec.Register("gong-test", func() {
...
// spawns gong with the given command line args. If the args don't set --datadir, the
// child g gets a temporary data directory.
func runGong(t *testing.T, args ...string) *testgong {
	tt := &testgong{}
	...
	for i, arg := range args {
		...
		case "--miner.ongerbase":
			if i < len(args)-1 {
				tt.Orangerbase = args[i+1]
			}
		}
	}
	if tt.Datadir == "" {
	...
	// Boot "gong". This actually runs the test binary but the TestMain
	// function will prevent any tests from running.
	tt.Run("gong-test", args...)
