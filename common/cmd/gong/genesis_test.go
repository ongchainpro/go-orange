...
// Tests that initializing Gong with a custom genesis block and chain definitions
// work properly.
func TestCustomGenesis(t *testing.T) {
	for i, tt := range customGenesisTests {
		...
		runGong(t, "--datadir", datadir, "init", json).WaitExit()

		// Query the custom genesis block
		gong := runGong(t, "--networkid", "1337", "--syncmode=full",
			"--datadir", datadir, "--maxpeers", "0", "--port", "0",
			"--nodiscover", "--nat", "none", "--ipcdisable",
			"--exec", tt.query, "console")
		gong.ExpectRegexp(tt.result)
		gong.ExpectExit()
	}
}
