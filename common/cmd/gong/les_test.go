...
func (g *gongrpc) killAndWait() {
	g.gong.Kill()
	g.gong.WaitExit()
}

func (g *gongrpc) callRPC(result interface{}, Method string, args ...interface{}) {
	if err := g.rpc.Call(&result, Method, args...); err != nil {
		g.gong.Fatalf("callRPC %v: %v", Method, err)
	}
}

func (g *gongrpc) addPeer(peer *gongrpc) {
	g.gong.Logf("%v.addPeer(%v)", g.name, peer.name)
	enode := peer.getNodeInfo().Enode
	peerCh := make(chan *p2p.PeerEvent)
	sub, err := g.rpc.Subscribe(context.Background(), "admin", peerCh, "peerEvents")
	if err != nil {
		g.gong.Fatalf("subscribe %v: %v", g.name, err)
	}
	...
	select {
	case ev := <-peerCh:
		g.gong.Logf("%v received event: type=%v, peer=%v", g.name, ev.Type, ev.Peer)
	case err := <-sub.Err():
		g.gong.Fatalf("%v sub error: %v", g.name, err)
	case <-timeout:
		g.gong.Error("timeout adding peer after", dur)
	}
}

// Use this function instead of `g.nodeInfo` directly
func (g *gongrpc) getNodeInfo() *p2p.NodeInfo {
	if g.nodeInfo != nil {
		return g.nodeInfo
	}
...
func (g *gongrpc) waitSynced() {
	// Check if it's synced now
	var result interface{}
	g.callRPC(&result, "ong_syncing")
	syncing, ok := result.(bool)
	if ok && !syncing {
		g.gong.Logf("%v already synced", g.name)
		return
	}
	...
	if err != nil {
		g.gong.Fatalf("%v syncing: %v", g.name, err)
	}
	defer sub.Unsubscribe()
	timeout := time.After(4 * time.Second)
	select {
	case ev := <-ch:
		g.gong.Log("'syncing' event", ev)
		syncing, ok := ev.(bool)
		if ok && !syncing {
			break
		}
		g.gong.Log("Other 'syncing' event", ev)
	case err := <-sub.Err():
		g.gong.Fatalf("%v notification: %v", g.name, err)
		break
	case <-timeout:
		g.gong.Fatalf("%v timeout syncing", g.name)
		break
	}
}
...
func startGongWithIpc(t *testing.T, name string, args ...string) *gongrpc {
	ipcName := fmt.Sprintf("gong-%d.ipc", atomic.AddUint32(&nextIPC, 1))
	...
	var err error
	ipcpath := ipcEndpoint(ipcName, g.gong.Datadir)
...
func initGong(t *testing.T) string {
	args := []string{"--networkid=42", "init", "./testdata/clique.json"}
	t.Logf("Initializing gong: %v ", args)
	g := runGong(t, args...)
	datadir := g.Datadir
	g.WaitExit()
	return datadir
...
func startLightServer(t *testing.T) *gongrpc {
	datadir := initGong(t)
	t.Logf("Importing keys to gong")
	runGong(t, "--datadir", datadir, "--password", "./testdata/password.txt", "account", "import", "./testdata/key.prv", "--lightkdf").WaitExit()
	account := "0x02f0d131f1f97aef08aec6e3291b957d9efe7105"
	server := startGongWithIpc(t, "lightserver", "--allow-insecure-unlock", "--datadir", datadir, "--password", "./testdata/password.txt", "--unlock", account, "--mine", "--light.serve=100", "--light.maxpeers=1", "--nodiscover", "--nat=extip:127.0.0.1", "--verbosity=4")
	return server
}

func startClient(t *testing.T, name string) *gongrpc {
	datadir := initGong(t)
	return startGongWithIpc(t, name, "--datadir", datadir, "--nodiscover", "--syncmode=light", "--nat=extip:127.0.0.1", "--verbosity=4")
}
