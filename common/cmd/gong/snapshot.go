...
var (
	snapshotCommand = cli.Command{
		...
		Subcommands: []cli.Command{
			{
				Name:      "prune-state",
				Usage:     "Prune stale orange state data based on the snapshot",
				ArgsUsage: "<root>",
				...
				Description: `
gong snapshot prune-state <state-root>
...
during the use of Gong, please also specify it here for correct deletion. Otherwise
...
`,
			},
			{
				...
				Description: `
gong snapshot verify-state <state-root>
...
`,
			},
			{
				...
				Description: `
gong snapshot traverse-state <state-root>
...
`,
			},
			{
				...
				Description: `
gong snapshot traverse-rawstate <state-root>
...
func pruneState(ctx *cli.Context) error {
	...
	pruner, err := pruner.NewPruner(chaindb, chain.CurrentBlock().Header(), stack.ResolvePath(""), stack.ResolvePath(config.Ong.TrieCleanCacheJournal), ctx.GlobalUint64(utils.BloomFilterSizeFlag.Name))
	if err != nil {
