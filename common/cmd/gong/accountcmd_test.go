...
func TestAccountListEmpty(t *testing.T) {
	gong := runGong(t, "account", "list")
	gong.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gong := runGong(t, "account", "list", "--datadir", datadir)
	defer gong.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gong := runGong(t, "account", "new", "--lightkdf")
	defer gong.ExpectExit()
	gong.Expect(`
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	gong := runGong(t, "account", "import", keyfile, "-password", passwordFile)
	defer gong.ExpectExit()
	gong.Expect(expected)
}
...
func TestWalletImport(t *testing.T) {
	gong := runGong(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gong.ExpectExit()
	gong.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gong.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	gong := runMinimalGong(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gong.StderrText(), m) {
...
// https://github.com/ong2020/go-orange/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gong.SetTemplateFunc("keypath", func(file string) string {
