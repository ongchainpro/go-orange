...
// faucet is an Oranger faucet backed by a light client.
package main
...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Orange protocol")
	statsFlag   = flag.String("ongstats", "", "Ongstats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Orangers to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Orange and the faucet")
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Orangers", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
...
type request struct {
	...
	Account common.Address     `json:"account"` // Orange address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Orange light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Orange protocol stack
	client *ongclient.Client   // Client connection to the Orange chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*enode.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Orange light client protocol
	cfg := ongconfig.Defaults
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to register the Orange service: %w", err)
	}

	// Assemble the ongstats monitoring and reporting service'
	if stats != "" {
		if err := ongstats.New(stack, lesBackend.ApiBackend, lesBackend.Engine(), stats); err != nil {
			return nil, err
		}
	}
	...
	client := ongclient.NewClient(api)
...
// close terminates the Orange connection and tears down the faucet.
...
// apiHandler handles requests for Oranger grants and transaction statuses.
func (f *faucet) apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		// Retrieve the Orange address to fund, the requesting user and a profile picture
		var (
			id       string
			username string
			avatar   string
		...
		default:
			//lint:ignore ST1005 This error is to be displayed in the browser
			err = errors.New("Somonging funky happened, please open an issue at https://github.com/ong2020/go-orange/issues")
		}
		if err != nil {
			if err = sendError(wsconn, err); err != nil {
...
// the uniqueness identifier (user id/username), username, avatar URL and Orange address to fund on success.
func authTwitter(url string, tokenV1, tokenV2 string) (string, string, string, common.Address, error) {
	...
	// and scrape it for the Orange address and profile URL. We need to load
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", "", common.Address{}, errors.New("No Orange address found to fund")
	}
	var avatar string
...
// API, returning the user id, username, avatar URL and Orange address to fund on
// success.
...
// returning the username, avatar URL and Orange address to fund on success.
func authFacebook(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Orange address and profile URL.
	//
...
// authNoAuth tries to interpret a faucet request as a plain Orange address,
