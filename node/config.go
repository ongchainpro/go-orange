...
import (
	...
	"github.com/ong2020/go-orange/accounts/external"
	"github.com/ong2020/go-orange/accounts/keystore"
	"github.com/ong2020/go-orange/accounts/scwallet"
	"github.com/ong2020/go-orange/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gong is "gong". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gong", keep that.
	if name == "gong" || name == "gong-testnet" {
		name = "Gong"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gong" instances.
var isOldGongResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gong 1.4 are used if they exist.
	if warn, isOld := isOldGongResource[path]; isOld {
		oldpath := ""
		if c.name() == "gong" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGongResourceWarning, "Using deprecated resource file %s, please move this file to the 'gong' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
