...
// checking whonger the HTTP server is started correctly.
func TestStartRPC(t *testing.T) {
	type test struct {
		...
		wantReachable bool // whonger the HTTP server should be reachable at all
		wantHandlers  bool // whonger RegisterHandler handlers should be accessible
		wantRPC       bool // whonger JSON-RPC/HTTP should be accessible
		wantWS        bool // whonger JSON-RPC/WS should be accessible
	}
...
// checkBodyOK checks whonger the given HTTP URL responds with 200 OK and body "OK".
...
// checkRPC checks whonger JSON-RPC works against the given URL.
