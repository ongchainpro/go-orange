...
import (
	...
	"github.com/ong2020/go-orange/internal/testlog"
	"github.com/ong2020/go-orange/log"
)

// Tests whonger remote HTTP servers are correctly notified of new work.
func TestRemoteNotify(t *testing.T) {
	...
	// Create the custom ongash engine.
	ongash := NewTester([]string{server.URL}, false)
	defer ongash.Close()
	...
	ongash.Seal(nil, block, nil, nil)
	select {
	case work := <-sink:
		if want := ongash.SealHash(header).Hex(); work[0] != want {
...
func TestRemoteMultiNotify(t *testing.T) {
	...
	// Create the custom ongash engine.
	ongash := NewTester([]string{server.URL}, false)
	ongash.config.Log = testlog.Logger(t, log.LvlWarn)
	defer ongash.Close()
...
// Tests whonger stale solutions are correctly processed.
func TestStaleSubmission(t *testing.T) {
	ongash := NewTester(nil, true)
	defer ongash.Close()
	api := &API{ongash}
	...
	for id, c := range testcases {
		for _, h := range c.headers {
			ongash.Seal(nil, types.NewBlockWithHeader(h), results, nil)
		}
		if res := api.SubmitWork(fakeNonce, ongash.SealHash(c.headers[c.submitIndex]), fakeDigest); res != c.submitRes {
		...
		case <-time.NewTimer(time.Second).C:
			t.Errorf("case %d fetch ongash result timeout", id+1)
		}
	}
}
