...
import (
	...
	"github.com/ong2020/go-orange/accounts/scwallet"
...
// PublicOrangeAPI provides an API to access Orange related information.
// It offers only Methods that operate on public data that is freely available to anyone.
type PublicOrangeAPI struct {
	b Backend
}

// NewPublicOrangeAPI creates a new Orange protocol API.
func NewPublicOrangeAPI(b Backend) *PublicOrangeAPI {
	return &PublicOrangeAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicOrangeAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
...
func (s *PublicOrangeAPI) Syncing() (interface{}, error) {
...
// Sign calculates an Orange ECDSA signature for:
...
//
// https://github.com/ong2020/go-orange/wiki/Management-APIs#personal_sign
...
// Note, this function is compatible with ong_sign and personal_sign. As such it recovers
// the address of:
// hash = keccak256("\x19Orange Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be 27 or 28 for legacy reasons.
//
// https://github.com/ong2020/go-orange/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid Orange signature (V is not 27 or 28)")
	}
...
// Unpair deletes a pairing between wallet and gong.
...
// PublicBlockChainAPI provides an API to access the Orange blockchain.
// It offers only Methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Orange blockchain API.
...
//
// https://github.com/ong2020/wiki/wiki/JSON-RPC#ong_sign
...
// Version returns the current orange protocol version.
func (s *PublicNetAPI) Version() string {
	return fmt.Sprintf("%d", s.networkVersion)
}

// checkTxFee is an internal function used to check whonger the fee of
