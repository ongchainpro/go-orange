...
// support all the operations needed by the Orange chain protocols.
type txPool interface {
	// Has returns an indicator whonger txpool has a transaction
...
type handlerConfig struct {
	Database   ongdb.Database            // Database for direct sync insertions
	Chain      *core.BlockChain          // Blockchain to serve data from
	TxPool     txPool                    // Transaction pool to propagate from
	Network    uint64                    // Network identifier to adfvertise
	Sync       downloader.SyncMode       // Whonger to fast or full sync
...
type handler struct {
	...
	fastSync  uint32 // Flag whonger fast sync is enabled (gets disabled if we already have blocks)
	snapSync  uint32 // Flag whonger fast sync should operate on top of the snap protocol
	acceptTxs uint32 // Flag whonger we're considered synchronised (enables transaction processing)

	checkpointNumber uint64      // Block number for the sync progress validator to cross reference
	checkpointHash   common.Hash // Block hash for the sync progress validator to cross reference

	database ongdb.Database
	txpool   txPool
...
func newHandler(config *handlerConfig) (*handler, error) {
	...
	inserter := func(blocks types.Blocks) (int, error) {
		...
		// out a way yet where nodes can decide unilaterally whonger the network is new
...
func (h *handler) runOngPeer(peer *ong.Peer, handler ong.Handler) error {
	...
	if err := peer.Handshake(h.networkID, td, hash, genesis.Hash(), forkID, h.forkFilter); err != nil {
		peer.Log().Debug("Orange handshake failed", "err", err)
		return err
	}
	...
	peer.Log().Debug("Orange peer connected", "name", peer.Name())

	// Register the peer locally
	if err := h.peers.registerPeer(peer, snap); err != nil {
		peer.Log().Error("Orange peer registration failed", "err", err)
		return err
	}
...
func (h *handler) removePeer(id string) {
	...
	if peer == nil {
		logger.Error("Orange peer removal failed", "err", errPeerNotRegistered)
		return
	}
	// Remove the `ong` peer if it exists
	logger.Debug("Removing Orange peer", "snap", peer.snapExt != nil)
	...
	if err := h.peers.unregisterPeer(id); err != nil {
		logger.Error("Orange peer removal failed", "err", err)
	}
...
func (h *handler) Stop() {
	...
	log.Info("Orange protocol stopped")
}
