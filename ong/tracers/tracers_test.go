...
import (
	...
	"github.com/ong2020/go-orange/tests"
)

// To generate a new callTracer test, copy paste the makeTest Method below into
// a Gong console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = ong.getBlock(ong.getTransaction(tx).blockHash);
  var genesis = ong.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.ong.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  ong.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
