...
// testOngHandler is a mock event handler to listen for inbound network requests
// on the `ong` protocol and convert them into a more easily testable form.
type testOngHandler struct {
...
func (h *testOngHandler) PeerInfo(enode.ID) interface{}        { panic("not used in tests") }

func (h *testOngHandler) Handle(peer *ong.Peer, packet ong.Packet) error {
	switch packet := packet.(type) {
	case *ong.NewBlockPacket:
...
func testForkIDSplit(t *testing.T, protocol uint) {
	...
	peerNoFork := ong.NewPeer(protocol, p2p.NewPeer(enode.ID{1}, "", nil), p2pNoFork, nil)
	peerProFork := ong.NewPeer(protocol, p2p.NewPeer(enode.ID{2}, "", nil), p2pProFork, nil)
	defer peerNoFork.Close()
	defer peerProFork.Close()

	errc := make(chan error, 2)
	go func(errc chan error) {
		errc <- ongNoFork.runOngPeer(peerProFork, func(peer *ong.Peer) error { return nil })
	}(errc)
	go func(errc chan error) {
		errc <- ongProFork.runOngPeer(peerNoFork, func(peer *ong.Peer) error { return nil })
	}(errc)
...
func testRecvTransactions(t *testing.T, protocol uint) {
	...
	src := ong.NewPeer(protocol, p2p.NewPeer(enode.ID{1}, "", nil), p2pSrc, handler.txpool)
	sink := ong.NewPeer(protocol, p2p.NewPeer(enode.ID{2}, "", nil), p2pSink, handler.txpool)
	defer src.Close()
	defer sink.Close()

	go handler.handler.runOngPeer(sink, func(peer *ong.Peer) error {
		return ong.Handle((*ongHandler)(handler.handler), peer)
	})
...
func testSendTransactions(t *testing.T, protocol uint) {
	...
	go handler.handler.runOngPeer(src, func(peer *ong.Peer) error {
		return ong.Handle((*ongHandler)(handler.handler), peer)
	})
	// Run the handshake locally to avoid spinning up a source handler
	var (
	...
	for len(seen) < len(insert) {
		...
		case 63, 64:
			select {
			case <-anns:
				t.Errorf("tx announce received on pre ong/65")
			case txs := <-bcasts:
				for _, tx := range txs {
		...
		case 65:
			...
			case <-bcasts:
				t.Errorf("initial tx broadcast received on post ong/65")
			}
...
func testTransactionPropagation(t *testing.T, protocol uint) {
	...
	for i, sink := range sinks {
		...
		sourcePeer := ong.NewPeer(protocol, p2p.NewPeer(enode.ID{byte(i)}, "", nil), sourcePipe, source.txpool)
		sinkPeer := ong.NewPeer(protocol, p2p.NewPeer(enode.ID{0}, "", nil), sinkPipe, sink.txpool)
		defer sourcePeer.Close()
		defer sinkPeer.Close()

		go source.handler.runOngPeer(sourcePeer, func(peer *ong.Peer) error {
			return ong.Handle((*ongHandler)(source.handler), peer)
		})
		go sink.handler.runOngPeer(sinkPeer, func(peer *ong.Peer) error {
			return ong.Handle((*ongHandler)(sink.handler), peer)
		})
	}
...
func testCheckpointChallenge(t *testing.T, syncmode downloader.SyncMode, checkpoint bool, timeout bool, empty bool, match bool, drop bool) {
	...
	local := ong.NewPeer(ong.ONG32, p2p.NewPeer(enode.ID{1}, "", nil), p2pLocal, handler.txpool)
	remote := ong.NewPeer(ong.ONG32, p2p.NewPeer(enode.ID{2}, "", nil), p2pRemote, handler.txpool)
	defer local.Close()
	defer remote.Close()

	go handler.handler.runOngPeer(local, func(peer *ong.Peer) error {
		return ong.Handle((*ongHandler)(handler.handler), peer)
	})
...
func testBroadcastBlock(t *testing.T, peers, bcasts int) {
	...
	sinks := make([]*testOngHandler, peers)
	for i := 0; i < len(sinks); i++ {
		sinks[i] = new(testOngHandler)
	}
	// Interconnect all the sink handlers with the source handler
	var (
	...
	for i, sink := range sinks {
		...
		sourcePeer := ong.NewPeer(ong.ONG32, p2p.NewPeer(enode.ID{byte(i)}, "", nil), sourcePipe, nil)
		sinkPeer := ong.NewPeer(ong.ONG32, p2p.NewPeer(enode.ID{0}, "", nil), sinkPipe, nil)
		defer sourcePeer.Close()
		defer sinkPeer.Close()
...
func testBroadcastMalformedBlock(t *testing.T, protocol uint) {
	...
	src := ong.NewPeer(protocol, p2p.NewPeer(enode.ID{1}, "", nil), p2pSrc, source.txpool)
	sink := ong.NewPeer(protocol, p2p.NewPeer(enode.ID{2}, "", nil), p2pSink, source.txpool)
	defer src.Close()
	defer sink.Close()

	go source.handler.runOngPeer(src, func(peer *ong.Peer) error {
		return ong.Handle((*ongHandler)(source.handler), peer)
	})
