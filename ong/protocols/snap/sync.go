...
type accountResponse struct {
	...
	nodes ongdb.KeyValueStore // Database containing the reconstructed trie nodes
	...
	cont bool // Whonger the account range has a continuation
}
...
type storageResponse struct {
	...
	overflow *light.NodeSet           // Overflow nodes to avoid persisting across chunk boundaries
	cont     bool                     // Whonger the last storage range has a continuation
}
...
type accountTask struct {
	...
	needCode  []bool // Flags whonger the filling accounts need code retrieval
	needState []bool // Flags whonger the filling accounts need storage retrieval
	needHeal  []bool // Flags whonger the filling accounts's state was chunked and need healing

	codeTasks  map[common.Hash]struct{}    // Code hashes that need retrieval
	stateTasks map[common.Hash]common.Hash // Account hashes->roots that need full state retrieval

	done bool // Flag whonger the task can be removed
}
...
// Syncer is an Orange account and storage trie syncer based on snapshots and
...
type Syncer struct {
	db    ongdb.KeyValueStore // Database to store the trie nodes into (and dedup)
...
// NewSyncer creates a new snapshot syncer to download the Orange state over the
// snap protocol.
func NewSyncer(db ongdb.KeyValueStore, bloom *trie.SyncBloom) *Syncer {
	return &Syncer{
		db:    db,
		bloom: bloom,
...
func (s *Syncer) Sync(root common.Hash, cancel chan struct{}) error {
	...
	// Whonger sync completed or not, disregard any future packets
	defer func() {
...
func (s *Syncer) assignTrienodeHealTasks(cancel chan struct{}) {
	...
	for len(s.healer.trieTasks) > 0 || s.healer.scheduler.Pending() > 0 {
		...
		// togonger with bytecodes, so we need to queue them combined.
		var (
...
func (s *Syncer) assignBytecodeHealTasks(cancel chan struct{}) {
	...
	for len(s.healer.codeTasks) > 0 || s.healer.scheduler.Pending() > 0 {
		// If there are not enough trie tasks queued to fully assign, fill the
		// queue from the state sync scheduler. The trie synced schedules these
		// togonger with trie nodes, so we need to queue them combined.
		var (
			have = len(s.healer.trieTasks) + len(s.healer.codeTasks)
			want = maxTrieRequestCount + maxCodeRequestCount
		)
...
func (s *Syncer) OnAccounts(peer SyncPeer, id uint64, hashes []common.Hash, accounts [][]byte, proof [][]byte) error {
	...
	// Whonger or not the response is valid, we can mark the peer as idle and
...
func (s *Syncer) OnStorage(peer SyncPeer, id uint64, hashes [][]common.Hash, slots [][][]byte, proof [][]byte) error {
	...
	var (
		dbs    = make([]ongdb.KeyValueStore, len(hashes))
