...
var (
	...
	// testAddr is the Orange address of the tester account.
	testAddr = crypto.PubkeyToAddress(testKey.PublicKey)
)

// testBackend is a mock implementation of the live Orange message handler. Its
// purpose is to allow testing the request/reply workflows and wire serialization
// in the `ong` protocol without actually doing any data processing.
...
func newTestBackendWithGenerator(blocks int, generator func(int, *core.BlockGen)) *testBackend {
	...
	chain, _ := core.NewBlockChain(db, nil, params.TestChainConfig, ongash.NewFaker(), vm.Config{}, nil, nil)

	bs, _ := core.GenerateChain(params.TestChainConfig, chain.Genesis(), ongash.NewFaker(), db, blocks, generator)
