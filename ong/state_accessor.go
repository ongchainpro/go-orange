...
func (ong *Orange) stateAtBlock(block *types.Block, reexec uint64) (statedb *state.StateDB, release func(), err error) {
	...
	for i := uint64(0); i < reexec; i++ {
		...
		parent := ong.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
		if parent == nil {
	...
	for block.NumberU64() < origin {
		...
		if block = ong.blockchain.GetBlockByNumber(block.NumberU64() + 1); block == nil {
			return nil, nil, fmt.Errorf("block #%d not found", block.NumberU64()+1)
		}
		_, _, _, err := ong.blockchain.Processor().Process(block, statedb, vm.Config{})
		if err != nil {
			return nil, nil, fmt.Errorf("processing block %d failed: %v", block.NumberU64(), err)
		}
		// Finalize the state so any modifications are written to the trie
		root, err := statedb.Commit(ong.blockchain.Config().IsEIP158(block.Number()))
		if err != nil {
			return nil, nil, err
		}
...
func (ong *Orange) statesInRange(fromBlock, toBlock *types.Block, reexec uint64) (states []*state.StateDB, release func(), err error) {
	statedb, err := ong.blockchain.StateAt(fromBlock.Root())
	if err != nil {
		statedb, _, err = ong.stateAtBlock(fromBlock, reexec)
	}
	if err != nil {
		return nil, nil, err
	}
	...
	for i := fromBlock.NumberU64() + 1; i <= toBlock.NumberU64(); i++ {
		...
		// Retrieve the next block to regenerate and process it
		block := ong.blockchain.GetBlockByNumber(i)
		if block == nil {
		...
		statedb, err := ong.blockchain.StateAt(root)
		if err != nil {
			return nil, nil, fmt.Errorf("state reset after block %d failed: %v", block.NumberU64(), err)
		}
		states = append(states, statedb.Copy())
...
func (ong *Orange) stateAtTransaction(block *types.Block, txIndex int, reexec uint64) (core.Message, vm.BlockContext, *state.StateDB, func(), error) {
	...
	statedb, release, err := ong.stateAtBlock(parent, reexec)
	if err != nil {
	...
	signer := types.MakeSigner(ong.blockchain.Config(), block.Number())
	for idx, tx := range block.Transactions() {
		// Assemble the transaction call message and return if the requested offset
		msg, _ := tx.AsMessage(signer)
		txContext := core.NewEVMTxContext(msg)
		context := core.NewEVMBlockContext(block.Header(), ong.blockchain, nil)
		if idx == txIndex {
			return msg, context, statedb, release, nil
		}
		// Not yet the searched for transaction, execute on top of the current state
		vmenv := vm.NewEVM(context, txContext, statedb, ong.blockchain.Config(), vm.Config{})
