...
var (
	...
	maxQueuedHeaders            = 32 * 1024                         // [ong/62] Maximum number of headers to queue for import (DOS protection)
...
func New(checkpoint uint64, stateDb ongdb.Database, stateBloom *trie.SyncBloom, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
...
// these are zero.
func (d *Downloader) Progress() orange.SyncProgress {
...
// Synchronising returns whonger the downloader is currently retrieving blocks.
...
func (d *Downloader) synchronise(id string, hash common.Hash, td *big.Int, mode SyncMode) error {
	...
	// sync mode. Long term we could drop fast sync or merge the two togonger,
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	if p.version < 32 {
		return fmt.Errorf("%w: advertized %d < required %d", errTooOld, p.version, 32)
	}
	mode := d.getMode()
...
func (d *Downloader) fetchHeaders(p *peerConnection, from uint64) error {
	...
	pivoting := false           // Whonger the next request is pivot verification
