...
type downloadTester struct {
	...
	stateDb ongdb.Database // Database used by the tester for syncing from peers
	peerDb  ongdb.Database // Database of the peers containing all data
...
func testSyncProgress(t *testing.T, protocol uint, mode SyncMode) {
	...
	checkProgress(t, tester.downloader, "pristine", orange.SyncProgress{})
	...
	<-starting
	checkProgress(t, tester.downloader, "initial", orange.SyncProgress{
	...
	<-starting
	checkProgress(t, tester.downloader, "completing", orange.SyncProgress{
	...
	progress <- struct{}{}
	pending.Wait()
	checkProgress(t, tester.downloader, "final", orange.SyncProgress{
		StartingBlock: uint64(chain.len()/2 - 1),
...
func checkProgress(t *testing.T, d *Downloader, stage string, want orange.SyncProgress) {
	// Mark this Method as a helper to report errors at callsite, not in here
	t.Helper()
...
func testForkedSyncProgress(t *testing.T, protocol uint, mode SyncMode) {
	...
	<-starting
	checkProgress(t, tester.downloader, "forking", orange.SyncProgress{
