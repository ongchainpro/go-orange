...
// stateReq represents a batch of state fetch requests grouped togonger into
...
type stateReq struct {
	...
	dropped   bool                      // Flag whonger the peer dropped off early
}
...
func (s *stateSync) commit(force bool) error {
	if !force && s.bytesUncommitted < ongdb.IdealBatchSize {
		return nil
	}
	start := time.Now()
	...
	for _, p := range peers {
		...
		if len(nodes)+len(codes) > 0 {
			...
			case s.d.trackStateReq <- req:
				req.peer.FetchNodeData(append(nodes, codes...)) // Unified retrieval under ong/6x
			case <-s.cancel:
			case <-s.d.cancelCh:
			}
		}
...
// delivered. Returns whonger the peer actually managed to deliver anything of
...
// peer into the state trie, returning whonger anything useful was written or any
// error occurred.
