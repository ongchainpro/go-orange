...
func (h *handler) syncTransactions(p *ong.Peer) {
	...
	// The ong/65 protocol introduces proper transaction announcements, so instead
	// of dripping transactions across multiple peers, just send the entire list as
	// an announcement and let the remote side decide what they need (likely nothing).
	if p.Version() >= ong.ONG33 {
...
func (h *handler) txsyncLoop64() {
	...
	var (
		...
		sending = false               // whonger a send is active
	...
	send := func(s *txsync) {
		if s.p.Version() >= ong.ONG33 {
			panic("initial transaction syncer running on ong/65+")
		}
...
// chain head.
func (cs *chainSyncer) handlePeerEvent(peer *ong.Peer) bool {
	select {
...
// nextSyncOp determines whonger sync is required at this time.
...
func peerToSyncOp(mode downloader.SyncMode, p *ong.Peer) *chainSyncOp {
...
func (h *handler) doSync(op *chainSyncOp) error {
	if op.mode == downloader.FastSync || op.mode == downloader.SnapSync {
		...
		// txlookup limit.
		// The main concern here is: during the fast sync Gong won't index the
		// block(generate tx indices) before the HEAD-limit. But if user changes
		// the limit in the next fast sync(e.g. user kill Gong manually and
		// restart) then it will be hard for Gong to figure out the oldest block
		// has been indexed. So here for the user-experience wise, it's non-optimal
		// that user can't change limit during the fast sync. If changed, Gong
