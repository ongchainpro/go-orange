...
// Package ongstats implements the network stats reporting service.
package ongstats
...
import (
	...
	ongproto "github.com/ong2020/go-orange/ong/protocols/ong"
...
// backend encompasses the bare-minimum functionality needed for ongstats reporting
...
// Service implements an Orange netstats reporting daemon that pushes local
...
func New(node *node.Node, backend backend, engine consensus.Engine, url string) error {
	...
	node.RegisterLifecycle(ongstats)
	return nil
}
...
func (s *Service) login(conn *connWrapper) error {
	...
	var network string
	if info := infos.Protocols["ong"]; info != nil {
		network = fmt.Sprintf("%d", info.(*ongproto.NodeInfo).Network)
	} else {
...
func (s *Service) reportLatency(conn *connWrapper) error {
	// Send the current time to the ongstats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to ongstats", "latency", latency)
...
func (s *Service) reportBlock(conn *connWrapper, block *types.Block) error {
	...
	log.Trace("Sending new block to ongstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *connWrapper, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to ongstats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *connWrapper) error {
	...
	log.Trace("Sending pending transactions to ongstats", "count", pending)
...
func (s *Service) reportStats(conn *connWrapper) error {
	...
	log.Trace("Sending node details to ongstats")
