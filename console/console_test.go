...
func newTester(t *testing.T, confOverride func(*ongconfig.Config)) *tester {
	...
	// Create a networkless protocol stack and start an Orange service within
	stack, err := node.New(&node.Config{DataDir: workspace, UseLightweightKDF: true, Name: testInstance})
	if err != nil {
		t.Fatalf("failed to create node: %v", err)
	}
	ongConf := &ongconfig.Config{
		Genesis: core.DeveloperGenesisBlock(15, common.Address{}),
		Miner: miner.Config{
			Orangerbase: common.HexToAddress(testAddress),
		},
		Ongash: ongash.Config{
			PowMode: ongash.ModeTest,
		},
	}
	if confOverride != nil {
		confOverride(ongConf)
	}
	ongBackend, err := ong.New(stack, ongConf)
	if err != nil {
		t.Fatalf("failed to register Orange protocol: %v", err)
	}
