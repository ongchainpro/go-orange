...
import (
	...
	"github.com/ong2020/go-orange/internal/jsre"
	"github.com/ong2020/go-orange/internal/jsre/deps"
	"github.com/ong2020/go-orange/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client         // RPC client to execute Orange requests through
...
// interaction. The original web3 callbacks are stored in 'jong'. These will be called
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines ong.getBalance(ong.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Gong instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	message := "Welcome to the Gong JavaScript console!\n\n"

	// Print some generic Gong metadata
	if res, err := c.jsre.Run(`
		var message = "instance: " + web3.version.node + "\n";
		try {
			message += "coinbase: " + ong.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + ong.blockNumber + " (" + new Date(1000 * ong.getBlock(ong.blockNumber).timestamp) + ")\n";
		try {
