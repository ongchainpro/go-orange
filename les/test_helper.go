...
import (
	...
	"github.com/ong2020/go-orange/accounts/abi/bind/backends"
	"github.com/ong2020/go-orange/common"
	"github.com/ong2020/go-orange/common/mclock"
	"github.com/ong2020/go-orange/consensus/ongash"
	"github.com/ong2020/go-orange/contracts/checkpointoracle/contract"
	...
	"github.com/ong2020/go-orange/les/flowcontrol"
	...
	"github.com/ong2020/go-orange/p2p/nodestate"
...
func prepare(n int, backend *backends.SimulatedBackend) {
	...
	for i := 0; i < n; i++ {
		...
		case 0:
			...
			// bankUser transfers some onger to user1
		...
		case 1:
			...
			// bankUser transfers more onger to user1
		...
		case 2:
			...
			// bankUser transfer some onger to signer
			bankNonce, _ := backend.PendingNonceAt(ctx, bankAddr)
...
func testIndexers(db ongdb.Database, odr light.OdrBackend, config *light.IndexerConfig, disablePruning bool) []*core.ChainIndexer {
...
func newTestClientHandler(backend *backends.SimulatedBackend, odr *LesOdr, indexers []*core.ChainIndexer, db ongdb.Database, peers *serverPeerSet, ulcServers []string, ulcFraction int) *clientHandler {
	var (
		evmux  = new(event.TypeMux)
		engine = ongash.NewFaker()
		gspec  = core.Genesis{
			Config:   params.AllOngashProtocolChanges,
	...
	client := &LightOrange{
		lesCommons: lesCommons{
			...
			config:      &ongconfig.Config{LightPeers: 100, NetworkId: NetworkId},
			chainConfig: params.AllOngashProtocolChanges,
...
func newTestServerHandler(blocks int, indexers []*core.ChainIndexer, db ongdb.Database, clock mclock.Clock) (*serverHandler, *backends.SimulatedBackend) {
	var (
		gspec = core.Genesis{
			Config:   params.AllOngashProtocolChanges,
			Alloc:    core.GenesisAlloc{bankAddr: {Balance: bankFunds}},
