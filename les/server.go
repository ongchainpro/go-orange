...
type LesServer struct {
	...
	archiveMode bool // Flag whonger the orange node runs in archive mode.
...
func NewLesServer(node *node.Node, e ongBackend, config *ongconfig.Config) (*LesServer, error) {
	lesDb, err := node.OpenDatabase("les.server", 0, 0, "ong/db/lesserver/")
	if err != nil {
		return nil, err
	}
...
func (s *LesServer) ServiceInfo() (string, string) {
	return "les", "Orange light client service"
}
