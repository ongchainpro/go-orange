...
// the ong.BlockFetcher as the underlying fetcher but adding more additional
...
func newLightFetcher(chain *light.LightChain, engine consensus.Engine, peers *serverPeerSet, ulc *ulc, chaindb ongdb.Database, reqDist *requestDistributor, syncFn func(p *serverPeer)) *lightFetcher {
...
func (f *lightFetcher) mainloop() {
	...
	var (
		...
		syncing      bool        // Indicator whonger the client is syncing
	...
	// trustedHeader returns an indicator whonger the header is regarded as
...
//
// Note, we rely on the underlying ong/fetcher to retrieve and validate the
// response, so that we have to obey the rule of ong/fetcher which only accepts
