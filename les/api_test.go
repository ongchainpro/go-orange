...
import (
	...
	"github.com/ong2020/go-orange/p2p/simulations"
...
// similar requests at the maximum allowed rate and the test verifies whonger the
...
func getHead(ctx context.Context, t *testing.T, client *rpc.Client) (uint64, common.Hash) {
	...
	if err := client.CallContext(ctx, &res, "ong_getBlockByNumber", "latest", false); err != nil {
...
func testRequest(ctx context.Context, t *testing.T, client *rpc.Client) bool {
	...
	defer cancel()
	err := client.CallContext(c, &res, "ong_getBalance", addr, "latest")
	if err != nil {
...
func newLesServerService(ctx *adapters.ServiceContext, stack *node.Node) (node.Lifecycle, error) {
	...
	orange, err := ong.New(stack, &config)
	if err != nil {
		return nil, err
	}
	_, err = NewLesServer(stack, orange, &config)
	if err != nil {
		return nil, err
	}
	return orange, nil
