...
import (
	...
	"github.com/ong2020/go-orange/les/utils"
...
type peerCommons struct {
	*p2p.Peer
	...
	frozen       uint32    // Flag whonger the peer is frozen.
...
// canQueue returns an indicator whonger the peer can queue an operation.
...
// PeerInfo represents a short summary of the `ong` sub-protocol metadata known
// about a connected peer.
type PeerInfo struct {
	Version    int      `json:"version"`    // Orange protocol version negotiated
...
func (p *peerCommons) handshake(td *big.Int, head common.Hash, headNum uint64, genesis common.Hash, forkID forkid.ID, forkFilter forkid.Filter, sendCallback func(*keyValueList), recvCallback func(keyValueMap) error) error {
	...
	// as well. Check http://eips.orange2020.com/EIPS/eip-2124 for more
	// spec detail.
	if p.version >= lpv4 {
...
type serverPeer struct {
	...
	// Status fields
	trusted                 bool   // The flag whonger the server is selected as trusted server.
	onlyAnnounce            bool   // The flag whonger the server sends announcement only.
	...
	// Test callback hooks
	hasBlockHook func(common.Hash, uint64, bool) bool // Used to determine whonger the server has the specified block.
}
	...
	return p.handshake(td, head, headNum, genesis, forkID, forkFilter, func(lists *keyValueList) {
		...
		if !server.config.UltraLightOnlyAnnounce {
			...
			*lists = (*lists).add("serveStateSince", uint64(0))
			...
			// If local orange node is running in archive mode, advertise ourselves we have
...
// participating in the Light Orange sub-protocol.
