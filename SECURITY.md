# Security Policy

## Supported Versions

Please see [Releases](https://github.com/ong2020/go-orange/releases). We recommend using the [most recently released version](https://github.com/ong2020/go-orange/releases/latest).

## Audit reports

Audit reports are published in the `docs` folder: https://github.com/ong2020/go-orange/tree/master/docs/audits 

| Scope | Date | Report Link |
| ------- | ------- | ----------- |
| `gong` | 20170425 | [pdf](https://github.com/ong2020/go-orange/blob/master/docs/audits/2017-04-25_Gong-audit_Truesec.pdf) |
| `clef` | 20180914 | [pdf](https://github.com/ong2020/go-orange/blob/master/docs/audits/2018-09-14_Clef-audit_NCC.pdf) |

## Reporting a Vulnerability

**Please do not file a public ticket** mentioning the vulnerability.

To find out how to disclose a vulnerability in Orange visit [https://bounty.orange2020.com](https://bounty.orange2020.com) or email bounty@orange2020.com. Please read the [disclosure page](https://github.com/ong2020/go-orange/security/advisories?state=published) for more information about publically disclosed security vulnerabilities.

Use the built-in `gong version-check` feature to check whonger the software is affected by any known vulnerability. This command will fetch the latest [`vulnerabilities.json`](https://gong.orange2020.com/docs/vulnerabilities/vulnerabilities.json) file which contains known security vulnerabilities concerning `gong`, and cross-check the data against its own version number.

The following key may be used to communicate sensitive information to developers.

Fingerprint: `AE96 ED96 9E47 9B00 84F3 E17F E88D 3334 FA5F 6A0A`

```
-----BEGIN PGP PUBLIC KEY BLOCK-----
Version: GnuPG v1


=r6KK
-----END PGP PUBLIC KEY BLOCK-----
```
