...
// This test checks whonger calls exceeding the request size limit are rejected.
...
func TestClientWebsocketPing(t *testing.T) {
	...
	sub, err := client.OngSubscribe(ctx, resultChan, "foo")
	if err != nil {
		t.Fatalf("client subscribe error: %v", err)
	}

	// Wait for the context's deadline to be reached before proceeding.
	// This is important for reproducing https://github.com/ong2020/go-orange/issues/19798
	<-ctx.Done()
	close(sendPing)
...
func wsPingTestHandler(t *testing.T, conn *websocket.Conn, shutdown, sendPing <-chan struct{}) {
	// Canned responses for the ong_subscribe call in TestClientWebsocketPing.
	const (
		subResp   = `{"jsonrpc":"2.0","id":1,"result":"0x00"}`
		subNotify = `{"jsonrpc":"2.0","Method":"ong_subscription","params":{"subscription":"0x00","result":1}}`
	)
