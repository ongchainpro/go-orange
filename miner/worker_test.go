...
var (
	...
	ongashChainConfig *params.ChainConfig
...
func init() {
	...
	ongashChainConfig = params.TestChainConfig
...
func newTestWorkerBackend(t *testing.T, chainConfig *params.ChainConfig, engine consensus.Engine, db ongdb.Database, n int) *testWorkerBackend {
...
func newTestWorker(t *testing.T, chainConfig *params.ChainConfig, engine consensus.Engine, db ongdb.Database, blocks int) (*worker, *testWorkerBackend) {
	backend := newTestWorkerBackend(t, chainConfig, engine, db, blocks)
	backend.txPool.AddLocals(pendingTxs)
	w := newWorker(testConfig, chainConfig, engine, backend, new(event.TypeMux), nil, false)
	w.setOrangerbase(testBankAddress)
	return w, backend
}

func TestGenerateBlockAndImportOngash(t *testing.T) {
...
func TestEmptyWorkOngash(t *testing.T) {
	testEmptyWork(t, ongashChainConfig, ongash.NewFaker())
}
...
func TestStreamUncleBlock(t *testing.T) {
	ongash := ongash.NewFaker()
	defer ongash.Close()

	w, b := newTestWorker(t, ongashChainConfig, ongash, rawdb.NewMemoryDatabase(), 1)
	defer w.close()
...
func TestRegenerateMiningBlockOngash(t *testing.T) {
	testRegenerateMiningBlock(t, ongashChainConfig, ongash.NewFaker())
}
...
func TestAdjustIntervalOngash(t *testing.T) {
	testAdjustInterval(t, ongashChainConfig, ongash.NewFaker())
}
