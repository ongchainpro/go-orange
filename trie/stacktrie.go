...
func stackTrieFromPool(db ongdb.KeyValueWriter) *StackTrie {
...
type StackTrie struct {
	...
	db        ongdb.KeyValueWriter // Pointer to the commit db, can be nil
}

// NewStackTrie allocates and initializes an empty trie.
func NewStackTrie(db ongdb.KeyValueWriter) *StackTrie {
	return &StackTrie{
		nodeType: emptyNode,
		db:       db,
	}
...
func newLeaf(ko int, key, val []byte, db ongdb.KeyValueWriter) *StackTrie {
...
func newExt(ko int, key []byte, child *StackTrie, db ongdb.KeyValueWriter) *StackTrie {
	st := stackTrieFromPool(db)
	st.nodeType = extNode
	st.keyOffset = ko
	st.key = append(st.key, key[ko:]...)
