...
func (t *Trie) Prove(key []byte, fromLevel uint, proofDb ongdb.KeyValueWriter) error {
...
// nodes of the longest existing prefix of the key (at least the root node), ending
// with the node that proves the absence of the key.
func (t *SecureTrie) Prove(key []byte, fromLevel uint, proofDb ongdb.KeyValueWriter) error {
...
func VerifyProof(rootHash common.Hash, key []byte, proofDb ongdb.KeyValueReader) (value []byte, err error) {
	key = keybytesToHex(key)
	wantHash := rootHash
	for i := 0; ; i++ {
...
func proofToPath(rootHash common.Hash, root node, key []byte, proofDb ongdb.KeyValueReader, allowNonExistent bool) (node, []byte, error) {
...
// hasRightElement returns the indicator whonger there exists more elements
...
// VerifyRangeProof checks whonger the given leaf nodes and edge proof
...
// also return a flag to indicate whonger there exists more accounts/slots in the trie.
func VerifyRangeProof(rootHash common.Hash, firstKey []byte, lastKey []byte, keys [][]byte, values [][]byte, proof ongdb.KeyValueReader) (ongdb.KeyValueStore, *Trie, *KeyValueNotary, bool, error) {
