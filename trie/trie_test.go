...
func TestReplication(t *testing.T) {
	...
	vals := []struct{ k, v string }{
		...
		{"doge", "coin"},
		{"dog", "puppy"},
		{"somongingveryoddindeedthis is", "myothernodedata"},
	}
	for _, val := range vals {
		updateString(trie, val.k, val.v)
	}
...
func (s *spongeDb) NewBatch() ongdb.Batch                    { return &spongeBatch{s} }
...
func (s *spongeDb) NewIterator(prefix []byte, start []byte) ongdb.Iterator { panic("implement me") }
...
func (b *spongeBatch) Replay(w ongdb.KeyValueWriter) error { return nil }
...
// to check whonger changes to the trie modifies the write order or data in any way.
