...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gong.LightScryptN, Gong.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.orange", new NodeConfig());
			node.start();
			...
			OrangeClient ec = node.getOrangeClient();
	...
	// signatures too. Regression test for go-orange issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogonger
	...
	workspace, err := ioutil.TempDir("", "gong-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/orange/gongtest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gong and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.orange", "github.com/ong2020/go-orange/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "gong.aar"), "gong.aar")

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "orange", "gongtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
