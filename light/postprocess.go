...
import (
	...
	"github.com/ong2020/go-orange/common/bitutil"
...
func GetChtRoot(db ongdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
...
func StoreChtRoot(db ongdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
type ChtIndexerBackend struct {
	disablePruning       bool
	diskdb, trieTable    ongdb.Database
	odr                  OdrBackend
...
func NewChtIndexer(db ongdb.Database, odr OdrBackend, size, confirms uint64, disablePruning bool) *core.ChainIndexer {
...
func GetBloomTrieRoot(db ongdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func StoreBloomTrieRoot(db ongdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func NewBloomTrieIndexer(db ongdb.Database, odr OdrBackend, parentSize, size uint64, disablePruning bool) *core.ChainIndexer {
