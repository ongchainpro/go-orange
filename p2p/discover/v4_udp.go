...
import (
	...
	"github.com/ong2020/go-orange/p2p/discover/v4wire"
...
type replyMatcher struct {
	...
	// reply was acceptable. The second return value indicates whonger the callback should
...
type reply struct {
	...
	ip   net.IP
	data v4wire.Packet
	// loop indicates whonger there was
...
// whonger any matcher considered the packet acceptable.
...
func (t *UDPv4) loop() {
	...
	for {
		...
		case r := <-t.gotreply:
			var matched bool // whonger any replyMatcher considered the reply acceptable.
			for el := plist.Front(); el != nil; el = el.Next() {
...
type packetHandlerV4 struct {
	...
	// preverify checks whonger the packet is valid and should be handled at all.
