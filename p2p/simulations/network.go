...
import (
	...
	"github.com/ong2020/go-orange/p2p/simulations/adapters"
)
...
// starting nodes and connecting them togonger.
//
...
// Connect connects two nodes togonger by calling the "admin_addPeer" RPC
...
// GetRandomNode returns a random node on the network, regardless of whonger it is running or not
...
// NOTE:
// it also checks whonger there has been recent attempt to connect the peers
...
type Node struct {
	...
	// up tracks whonger or not the node is running
	up   bool
	upMu *sync.RWMutex
}
...
// Up returns whonger the node is currently up (online)
func (n *Node) Up() bool {
	n.upMu.RLock()
...
func (n *Node) UnmarshalJSON(raw []byte) error {
	...
	// Ticket: https://github.com/ongersphere/go-orange/issues/1177
	var node struct {
		Config *adapters.NodeConfig `json:"config,omitempty"`
		Up     bool                 `json:"up"`
	}
...
type Conn struct {
	...
	// Up tracks whonger or not the connection is active
	Up bool `json:"up"`
...
// nodesUp returns whonger both nodes are currently up
