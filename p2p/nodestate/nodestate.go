...
type (
	...
	// function description, along with whonger it should run inside an operation callback.
	NodeStateMachine struct {
		started, closed     bool
		lock                sync.Mutex
		clock               mclock.Clock
		db                  ongdb.KeyValueStore
		dbNodeKey           []byte
...
// flagOp implements binary flag operations and also checks whonger the operands belong to the same setup
...
func NewNodeStateMachine(db ongdb.KeyValueStore, dbKey []byte, clock mclock.Clock, setup *Setup) *NodeStateMachine {
	if setup.flags == nil {
...
// stateMask checks whonger the set of flags belongs to the same setup and returns its internal bit mask
...
// fieldIndex checks whonger the field belongs to the same setup and returns its internal index
...
// checkStarted checks whonger the state machine has already been started and panics otherwise.
...
// opCheck checks whonger an operation is active
